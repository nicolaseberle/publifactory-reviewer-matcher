...
def parallel_func(val):
    ...
    if val["_source"]["journalName"]:
        ...
        if val["_source"]["journalName"].lower() in journal_classif['Title'].tolist():
            sub_cat = \
            journal_classif[journal_classif["Title"] == val["_source"]["journalName"].lower()]["Categories"].values[0]
            fields = \
            journal_classif[journal_classif["Title"] == val["_source"]["journalName"].lower()]["Fields"].values[0]
            ...
            if test[0].lower() == "the":
                ...
                val["_source"]["journalName"] = ' '.join(test)
                if val["_source"]["journalName"].lower() in journal_classif['Title'].tolist():
                    sub_cat = journal_classif[journal_classif["Title"] == val["_source"]["journalName"].lower()][
                        "Categories"].values[0]
                    fields = \
                    journal_classif[journal_classif["Title"] == val["_source"]["journalName"].lower()]["Fields"].values[
                        0]
