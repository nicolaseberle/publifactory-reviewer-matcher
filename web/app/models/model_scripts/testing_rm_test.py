            ...
            if len(auth["ids"]) > 0:
                ...
                # If true, we add score and article
                    if res["id"] == auth["ids"][0]:
                        res["score"] += i[1]
                        res["scorePond"] += newScore
                        res["article"].append(article["title"]+"("+str(article["year"])+")")
                ...
                if not temp:
                    resultats.append(
                        {"verification": auth["verif"], "name": auth["name"], "id": auth["ids"][0], "affiliation": auth["affiliation"], "conflit": "soon",
                         "score": i[1], "scorePond": newScore, "article": [article["title"]+"("+str(article["year"])+")"], "abstract": [article["paperAbstract"]],"contact": auth["contact"]})
